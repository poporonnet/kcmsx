...
const generateKcmsfEnv = (env: { serverUrl: string }) => {
  const envPath = path.join(packagePath, "kcmsf/.env.production");
  const envContent = `VITE_API_URL=${env.serverUrl}\n`;
...
const main = async () => {
  ...
  const clientUrl = await getOrQuit(
    text({
      message:
        "What will be the client URL for kcmsx? Leave it blank if you are unsure.",
      placeholder: "https://kcmsx.example.com",
      defaultValue: "",
    })
  );

  const serverUrl = await getOrQuit(
    text({
      message:
        "What will be the server URL for kcmsx? Leave it blank if you are unsure.",
      placeholder: "https://kcms.example.com",
      defaultValue: "",
    })
  );
