...
describe('FetchRunResultService', () => {
  ...
  it('MainMatchの走行結果が取得できる', async () => {
    const res = await fetchRunResultService.fetch('main', '900' as MainMatchID);
  ...
  it('PreMatchの走行結果が取得できる', async () => {
    const res = await fetchRunResultService.fetch('pre', '100' as PreMatchID);
    expect(Result.isErr(res)).toBe(false);
  ...
  it('MainMatchの走行結果が無ければエラーを返す', async () => {
    const res = await fetchRunResultService.fetch('main', '0' as MainMatchID);
    expect(Result.isErr(res)).toBe(true);
  });
  it('PreMatchの走行結果が無ければエラーを返す', async () => {
    const res = await fetchRunResultService.fetch('pre', '0' as PreMatchID);
    expect(Result.isErr(res)).toBe(true);
  });
});
